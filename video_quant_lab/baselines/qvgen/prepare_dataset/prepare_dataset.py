...
def check_height(x: Any) -> int:
    """Validate and parse height bucket values.

    Args:
        x: Height value to validate.

    Returns:
        Validated height as int.
    """
    x = int(x)
    if x % 16 != 0:
...
def check_width(x: Any) -> int:
    """Validate and parse width bucket values.

    Args:
        x: Width value to validate.

    Returns:
        Validated width as int.
    """
    x = int(x)
    if x % 16 != 0:
        raise argparse.ArgumentTypeError(
...
def check_frames(x: Any) -> int:
    """Validate and parse frame bucket values.

    Args:
        x: Frame count value to validate.

    Returns:
        Validated frame count as int.
    """
    x = int(x)
    if x % 4 != 0 and x % 4 != 1:
        raise argparse.ArgumentTypeError(
...
def get_args() -> Dict[str, Any]:
    ...
    Returns:
        Dict-like argparse Namespace with options.
    """
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to CSV file containing metadata about training data.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=49,
        help="Maximum number of frames in output video.",
    )
    parser.add_argument(
        "--max_sequence_length",
        type=int,
        default=226,
        help="Max sequence length of prompt embeddings.",
    )
    parser.add_argument(
        "--target_fps", type=int, default=8, help="Frame rate of output videos."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of videos to process at once in the VAE.",
    )
    parser.add_argument(
        "--num_decode_threads",
        type=int,
    ...
    parser.add_argument(
        "--num_artifact_workers",
        type=int,
        default=4,
        help="Number of worker threads for serializing artifacts.",
    )
...
def _get_t5_prompt_embeds(
    ...
    Args:
        ...
        text_encoder: T5 encoder instance.
        ...
        dtype: Target dtype.
        text_input_ids: Optional precomputed token IDs.
...
def encode_prompt(
    ...
    """Wrapper to encode prompts into embeddings.
...
def compute_prompt_embeddings(
    ...
    """Compute prompt embeddings with optional gradient tracking.
...
def save_image(image: torch.Tensor, path: pathlib.Path) -> None:
    ...
    Args:
        image: Image tensor in [-1, 1] range.
...
def save_video(video: torch.Tensor, path: pathlib.Path, fps: int = 8) -> None:
    """Save a video tensor to disk as an mp4.

    Args:
        video: Video tensor in [-1, 1] range.
        path: Output file path.
...
def save_metadata(metadata: Dict[str, Any], path: pathlib.Path) -> None:
    """Write metadata dict to disk as JSON.
...
def serialize_artifacts(
    ...
    """Serialize batch artifacts (images/videos/latents/prompts) to disk.

    Args:
        batch_size: Number of items in the batch.
        fps: Frame rate for videos.
        images_dir: Directory for images.
        image_latents_dir: Directory for image latents.
        videos_dir: Directory for videos.
        video_latents_dir: Directory for video latents.
        prompts_dir: Directory for prompt text files.
        prompt_embeds_dir: Directory for prompt embeddings.
        images: Image tensor batch.
        image_latents: Image latents batch.
        videos: Video tensor batch.
        video_latents: Video latents batch.
        prompts: Prompt strings.
        prompt_embeds: Prompt embeddings batch.
...
def save_intermediates(output_queue: queue.Queue) -> None:
    """Consume queued artifacts and serialize them.

    Args:
        output_queue: Queue of artifact payloads.
...
def main():
    ...
    if world_size > 1:
        ...
        if rank == world_size - 1:
            end_index = (
                original_dataset_size  # Make sure the last GPU gets the remaining data
            )
    ...
    if rank == 0:
        iterator = tqdm(
            dataloader,
            desc="Encoding",
            total=(rank_dataset_size + args.batch_size - 1) // args.batch_size,
        )
    else:
        iterator = dataloader
        ...
        try:
            ...
            if args.save_latents_and_embeddings:
                if args.use_slicing:
                    if args.save_image_latents:
                        encoded_slices = [
                            vae._encode(image_slice) for image_slice in images.split(1)
                        ]
                    ...
                    encoded_slices = [
                        vae._encode(video_slice) for video_slice in videos.split(1)
                    ]
            ...
            print("-------------------------")
            print(
                f"An exception occurred while processing data: {rank=}, {world_size=}, {step=}"
            )
            traceback.print_exc()
            print("-------------------------")
