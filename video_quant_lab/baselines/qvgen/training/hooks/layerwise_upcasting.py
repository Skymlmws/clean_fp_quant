...
class LayerwiseUpcastingHook(ModelHook):
    ...
    def __init__(
        self, storage_dtype: torch.dtype, compute_dtype: torch.dtype, non_blocking: bool
    ) -> None:
...
def apply_layerwise_upcasting(
    ...
    if skip_modules_classes is None and skip_modules_pattern is None:
        apply_layerwise_upcasting_hook(
            module, storage_dtype, compute_dtype, non_blocking
        )
        return

    should_skip = (
        skip_modules_classes is not None and isinstance(module, skip_modules_classes)
    ) or (
        skip_modules_pattern is not None
