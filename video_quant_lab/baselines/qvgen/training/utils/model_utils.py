...
def resolve_vae_cls_from_ckpt_path(ckpt_path, **kwargs):
    """Resolve the VAE class from a local checkpoint or Hub repo.

    Args:
        ckpt_path: Local path or Hub repo ID containing ``model_index.json``.
        **kwargs: Optional ``revision`` and ``cache_dir`` for Hub download.

    Returns:
        VAE class object imported from the model index metadata.
    """
