...
def should_perform_precomputation(precomputation_dir: Union[str, Path]) -> bool:
    """Check if precomputed latents/conditions need to be generated.

    Args:
        precomputation_dir: Directory containing precomputed artifacts.

    Returns:
        True if precomputation should run; otherwise False.
    """
    ...
    if conditions_dir.exists() and latents_dir.exists():
        ...
        if num_files_conditions > 0:
            logger.info(
                f"Found {num_files_conditions} precomputed conditions and latents."
            )
            return False
