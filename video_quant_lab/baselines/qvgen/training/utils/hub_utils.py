...
def save_model_card(
    ...
    """Create and save a Hugging Face model card with sample videos.

    Args:
        args: Training args namespace with output paths and metadata.
        repo_id: Hub repo ID to link in the card.
        videos: List of video paths or in-memory frames.
        validation_prompts: Prompts corresponding to the videos.
        fps: Frame rate for exported videos.

    Returns:
        None. Writes a ``README.md`` model card under ``args.output_dir``.
    """
    widget_dict = []
    ...
    if videos is not None and len(videos) > 0:
        for i, (video, validation_prompt) in enumerate(zip(videos, validation_prompts)):
            if not isinstance(video, str):
                export_to_video(
                    video, os.path.join(output_dir, f"final_video_{i}.mp4"), fps=fps
                )
            widget_dict.append(
                {
