...
def get_latest_ckpt_path_to_resume_from(
    ...
    """Resolve the checkpoint path and resume counters.

    Args:
        resume_from_checkpoint: Checkpoint path or "latest".
        num_update_steps_per_epoch: Steps per epoch for computing starting epoch.
        output_dir: Base output directory containing checkpoints.

    Returns:
        Tuple of (checkpoint_path, initial_global_step, global_step, first_epoch).
    """
...
def get_intermediate_ckpt_path(
    checkpointing_limit: int, step: int, output_dir: str
) -> str:
    """Compute a checkpoint path and enforce retention limits.
    ...
    Returns:
        Path to the checkpoint directory to save.
    """
    ...
    if checkpointing_limit is not None:
        ...
        if len(checkpoints) >= checkpointing_limit:
            ...
            checkpoints_to_remove = [
                os.path.join(output_dir, x) for x in checkpoints[0:num_to_remove]
            ]
