...
def perform_peft_patches() -> None:
    """Apply PEFT patches required for low-precision training."""
    _perform_patch_move_adapter_to_device_of_base_layer()


def _perform_patch_move_adapter_to_device_of_base_layer() -> None:
    """Patch BaseTunerLayer to avoid dtype casts on adapter weights."""
    ...
    BaseTunerLayer._move_adapter_to_device_of_base_layer = (
        _patched_move_adapter_to_device_of_base_layer(
            BaseTunerLayer._move_adapter_to_device_of_base_layer
        )
    )
...
def _patched_move_adapter_to_device_of_base_layer(func) -> None:
    """Wrap adapter move to suppress dtype conversions.

    Args:
        func: Original BaseTunerLayer move function.

    Returns:
        Wrapped function with dtype conversion disabled.
    """
...
class DisableTensorToDtype:
    """Context manager that ignores dtype in tensor.to calls."""
