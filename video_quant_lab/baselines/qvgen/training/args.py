...
class Args:
    ...
    For helpful information about arguments, run `python train.py --help`.
     ...
     add `python train.py --recommend_configs --model_name <model_name>` to recommend
    good training configs for a model after extensive testing.
     add `python train.py --memory_requirements --model_name <model_name>` to show
    ...
    w_group_size: int = -1
    w_cali: str = "minmax"
    w_use_grad_scaling: bool = False
    w_lr: float = 1e-6
    act_quantizer: str = "dynamic"
    act_bit: int = 8
    act_clip_ratio: float = 0.95
    use_aq: bool = False
    act_lr: float = 1e-2
    clip_group_num: int = 50
    round_func: str = "STE"
    round_group_num: int = 1
    round_lr: float = 1e-3
    qa_lora: bool = False
    qa_rank: int = 64
    qa_dropout: float = 0.0
    qa_alpha: float = 64
    qa_lora_grad_scaling: bool = True
    freeze_adaln: bool = False
    progress_rank: int = 32
    progress_iter: int = 10
    progress_cur_alpha_strategy: str = "cosine"
    progress_alpha_T: int = 200
    progress_warm_up: int = 200
    progress_training: bool = False
    progress_lr: float = 1e-6
    progress_fix: float = False

    # test
    wo_quant_adaln: bool = False
    wo_quant_random: bool = False
    wo_quant_num: int = -1
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "quantization_arguments": {
                ...
                "w_bit": self.w_bit,
                "w_granularity": self.w_granularity,
                "w_group_size": self.w_group_size,
                "w_cali": self.w_cali,
                "w_use_grad_scaling": self.w_use_grad_scaling,
                "w_lr": self.w_lr,
                ...
                "use_aq": self.use_aq,
                "act_lr": self.act_lr,
                "clip_group_num": self.clip_group_num,
                "round_func": self.round_func,
                "round_group_num": self.round_group_num,
                "round_lr": self.round_lr,
                "qa_lora": self.qa_lora,
                "qa_rank": self.qa_rank,
                "qa_dropout": self.qa_dropout,
                "qa_alpha": self.qa_alpha,
                "qa_lora_grad_scaling": self.qa_lora_grad_scaling,
                "wo_quant_adaln": self.wo_quant_adaln,
                "wo_quant_random": self.wo_quant_random,
                "wo_quant_num": self.wo_quant_num,
                "wo_quant_only_time_embedding": self.wo_quant_only_time_embedding,
                "wo_quant_only_norm": self.wo_quant_only_norm,
                "freeze_adaln": self.freeze_adaln,
                "progress_rank": self.progress_rank,
                "progress_iter": self.progress_iter,
                "progress_cur_alpha_strategy": self.progress_cur_alpha_strategy,
                "progress_alpha_T": self.progress_alpha_T,
                "progress_warm_up": self.progress_warm_up,
                "progrss_training": self.progress_training,
                "progress_lr": self.progress_lr,
                "progress_fix": self.progress_fix,
            },
        }


#  handle more informative messages
...
def parse_arguments() -> Args:
    ...
    Returns:
        Parsed Args instance populated from CLI flags.
    """
    ...
    if _IS_ARGUMENTS_REQUIRED:
        ...
        _add_quantization_arguments(parser)
...
def validate_args(args: Args):
    """Run validation checks on parsed arguments.

    Args:
        args: Parsed Args instance.

    Returns:
        None. Raises on validation errors.
    """
...
def _add_model_arguments(parser: argparse.ArgumentParser) -> None:
    """Register model-related CLI arguments.

    Args:
        parser: ArgumentParser to register flags on.
    ...
    parser.add_argument(
        ...
        type=str,
        default="bf16",
        help="Data type for the text encoder.",
    )
    parser.add_argument(
        "--text_encoder_2_dtype",
        type=str,
        default="bf16",
        help="Data type for the text encoder 2.",
    )
    parser.add_argument(
        "--text_encoder_3_dtype",
        type=str,
        default="bf16",
        help="Data type for the text encoder 3.",
    )
    parser.add_argument(
        "--transformer_dtype",
        type=str,
        default="bf16",
        help="Data type for the transformer model.",
    )
    parser.add_argument(
        "--vae_dtype", type=str, default="bf16", help="Data type for the VAE model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_teacher",
        action="store_true",
        help="Use teacher model instead of real data as supervision for training.",
    )


def _add_quantization_arguments(parser: argparse.ArgumentParser) -> None:
    """Register quantization-related CLI arguments.
    ...
    parser.add_argument(
        "--w_use_grad_scaling",
        action="store_true",
        help="Use gradient scaling for LSQ/LSQ+.",
    )
    parser.add_argument(
        "--w_cali",
        type=str,
        default="minmax",
        help="Calibration method for weights.",
        choices=["minmax", "mse"],
    )
    parser.add_argument(
        "--w_lr",
        type=float,
        default=1e-6,
        help="Learning rate for weight quantization (Ignore `--scale_lr`).",
    )
    parser.add_argument(
        "--act_quantizer",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="Clip ratio for activations.",
    )
    parser.add_argument(
        "--use_aq",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-2,
        help="Learning rate for activation clipping (Ignore `--scale_lr`).",
    )
    parser.add_argument(
        "--clip_group_num",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="STE",
        help="Round function for quantization.",
        choices=["STE", "AdaSTE", "LearnableAdaSTE"],
    )
    parser.add_argument(
        "--round_group_num",
        type=int,
        default=1,
        help="Round group number for quantization.",
    )
    parser.add_argument(
        "--round_lr",
        type=float,
        default=1e-3,
        help="Learning rate for round function (Ignore `--scale_lr`).",
    )
    parser.add_argument(
        "--qa_lora",
        action="store_true",
        help="Whether to use QA-LoRA.",
    )
    parser.add_argument(
        "--qa_rank",
        type=int,
        default=64,
        help="The rank of LoRA for QA-LoRA.",
    )
    parser.add_argument(
        "--qa_dropout",
        type=float,
        default=0.0,
        help="Dropout ratio of LoRA for QA-LoRA.",
    )
    parser.add_argument(
        "--qa_alpha",
        type=int,
        default=64,
        help="The alpha coefficient of LoRA for QA-LoRA.",
    )
    parser.add_argument(
        "--qa_lora_grad_scaling",
        action="store_true",
        help="Whether to scale the `lr` of QA-LoRA.",
    )
    parser.add_argument(
        "--wo_quant_adaln",
        action="store_true",
        help="Whether to quantize the AdaLN in the transformer.",
    )
    parser.add_argument(
        "--wo_quant_random",
        action="store_true",
        help="Whether to select the random number of layers w/o quantization in the transformer.",
    )
    parser.add_argument(
        "--wo_quant_num",
        type=int,
        default=-1,
        help="The number of layers w/o quantization in the transformer.",
    )
    parser.add_argument(
        "--wo_quant_only_time_embedding",
        action="store_true",
        help="Whether to w/o quantize only the time embedding in the transformer.",
    )
    parser.add_argument(
        "--wo_quant_only_norm",
        action="store_true",
        help="Whether to w/o quantize only the norm1/norm2 in the transformer.",
    )
    parser.add_argument(
        "--freeze_adaln",
        action="store_true",
        help="Whether to freeze adaln in the transformer.",
    )
    parser.add_argument(
        "--progress_rank",
        type=int,
        default=32,
        help="The rank of LoRA for progress.",
    )
    parser.add_argument(
        "--progress_iter",
        type=int,
        default=10,
        help="The number of iterations for progress.",
    )
    parser.add_argument(
        "--progress_cur_alpha_strategy",
        type=str,
        default="cosine",
        help="The strategy for progress.",
    ...
    parser.add_argument(
        "--progress_training",
        action="store_true",
        help="Whether to use progress training.",
    )
    parser.add_argument(
        "--progress_lr",
        type=float,
        default=1e-6,
        help="Learning rate for progressive training.",
    )
    parser.add_argument(
        "--progress_fix",
        action="store_true",
        help="Whether to train weights during warm-up.",
    )


def _add_dataset_arguments(parser: argparse.ArgumentParser) -> None:
    """Register dataset-related CLI arguments.
...
def _add_dataloader_arguments(parser: argparse.ArgumentParser) -> None:
    """Register dataloader-related CLI arguments.
...
def _add_diffusion_arguments(parser: argparse.ArgumentParser) -> None:
    """Register diffusion-related CLI arguments.
...
def _add_training_arguments(parser: argparse.ArgumentParser) -> None:
    """Register training-related CLI arguments.
    ...
    parser.add_argument(
        "--rank", type=int, default=64, help="The rank for LoRA matrices."
    )
    parser.add_argument(
        "--lora_alpha",
        type=int,
...
def _add_optimizer_arguments(parser: argparse.ArgumentParser) -> None:
    """Register optimizer-related CLI arguments.
...
def _add_validation_arguments(parser: argparse.ArgumentParser) -> None:
    """Register validation-related CLI arguments.
...
def _add_miscellaneous_arguments(parser: argparse.ArgumentParser) -> None:
    """Register miscellaneous CLI arguments.
    ...
    """
    parser.add_argument(
        "--tracker_name", type=str, default="qvgen", help="Project tracker name"
    )
    parser.add_argument(
        "--push_to_hub",
        action="store_true",
    ...
    parser.add_argument(
        "--name", type=str, default=None, help="The Name of the running pass for wandb"
    )


def _add_helper_arguments(parser: argparse.ArgumentParser) -> None:
    """Register helper/utility CLI arguments.
...
def _map_to_args_type(args: Dict[str, Any]) -> Args:
    """Convert an argparse Namespace to Args dataclass-like wrapper.

    Args:
        args: Parsed argparse Namespace or dict.

    Returns:
        Args instance with validated fields.
    """
    result_args = Args()
    ...
    result_args.layerwise_upcasting_storage_dtype = _DTYPE_MAP[
        args.layerwise_upcasting_storage_dtype
    ]
    result_args.layerwise_upcasting_skip_modules_pattern = (
        args.layerwise_upcasting_skip_modules_pattern
    )
    result_args.use_teacher = args.use_teacher

    # Dataset arguments
    if args.data_root is None and args.dataset_file is None:
        raise ValueError(
            "At least one of `data_root` or `dataset_file` should be provided."
        )
    ...
    result_args.image_resolution_buckets = (
        args.image_resolution_buckets or DEFAULT_IMAGE_RESOLUTION_BUCKETS
    )
    result_args.video_resolution_buckets = (
        args.video_resolution_buckets or DEFAULT_VIDEO_RESOLUTION_BUCKETS
    )
    ...
    # Validation arguments
    validation_prompts = (
        args.validation_prompts.split(args.validation_separator)
        if args.validation_prompts
        else []
    )
    validation_images = (
        args.validation_images.split(args.validation_separator)
        if args.validation_images
        else None
    )
    validation_videos = (
        args.validation_videos.split(args.validation_separator)
    ...
    result_args.name = (
        args.name if args.name is not None else args.output_dir.split("/")[-1]
    )

    # Quantization arguments
    result_args.w_quantizer = args.w_quantizer
    result_args.w_bit = args.w_bit
    result_args.w_granularity = args.w_granularity
    result_args.w_group_size = args.w_group_size
    result_args.w_use_grad_scaling = args.w_use_grad_scaling
    result_args.w_cali = args.w_cali
    result_args.act_bit = args.act_bit
    result_args.act_clip_ratio = args.act_clip_ratio
    result_args.use_aq = args.use_aq
    result_args.w_lr = args.w_lr
    result_args.act_lr = args.act_lr
    result_args.clip_group_num = args.clip_group_num
    result_args.act_quantizer = args.act_quantizer
    result_args.round_func = args.round_func
    result_args.round_group_num = args.round_group_num
    result_args.round_lr = args.round_lr
    result_args.qa_lora = args.qa_lora
    result_args.qa_rank = args.qa_rank
    result_args.qa_dropout = args.qa_dropout
    result_args.qa_alpha = args.qa_alpha
    result_args.qa_lora_grad_scaling = args.qa_lora_grad_scaling
    result_args.wo_quant_adaln = args.wo_quant_adaln
    result_args.wo_quant_random = args.wo_quant_random
    result_args.wo_quant_num = args.wo_quant_num
    result_args.wo_quant_only_time_embedding = args.wo_quant_only_time_embedding
    result_args.wo_quant_only_norm = args.wo_quant_only_norm
    result_args.freeze_adaln = args.freeze_adaln
    result_args.progress_alpha_T = args.progress_alpha_T
    result_args.progress_rank = args.progress_rank
    result_args.progress_iter = args.progress_iter
    result_args.progress_cur_alpha_strategy = args.progress_cur_alpha_strategy
    result_args.progress_warm_up = args.progress_warm_up
    result_args.progress_training = args.progress_training
    result_args.progress_lr = args.progress_lr
    result_args.progress_fix = args.progress_fix

    return result_args


def _validated_model_args(args: Args):
    """Validate model-related argument combinations.

    Args:
        args: Parsed Args instance.

    Returns:
        None. Raises on invalid configurations.
    """
...
def _validate_training_args(args: Args):
    """Validate training-related argument combinations.
...
def _validate_validation_args(args: Args):
    """Validate validation-related argument combinations.
    ...
    ), "Validation prompts are required for validation"
...
def _display_helper_messages(args: argparse.Namespace):
    """Print helper messages for model listing or recommendations.

    Args:
        args: Parsed argparse Namespace.

    Returns:
        None. May exit the process after printing.
    """
    if args.list_models:
