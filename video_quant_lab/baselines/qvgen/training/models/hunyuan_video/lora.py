...
def load_condition_models(
    ...
    **kwargs,
    ...
    """Load tokenizers and text encoders for HunyuanVideo.

    Args:
        model_id: Hugging Face model ID.
        text_encoder_dtype: Dtype for LLaMA text encoder.
        text_encoder_2_dtype: Dtype for CLIP text encoder.
        revision: Optional model revision.
        cache_dir: Optional cache directory.
        **kwargs: Unused additional args.

    Returns:
        Dict containing tokenizers and text encoders.
    """
    ...
    text_encoder_2 = CLIPTextModel.from_pretrained(
        ...
        torch_dtype=text_encoder_2_dtype,
        revision=revision,
        cache_dir=cache_dir,
    )
    return {
...
def load_latent_models(
    ...
    **kwargs,
    ...
    """Load the VAE for HunyuanVideo latents.
...
def load_diffusion_models(
    ...
    **kwargs,
    ...
    """Load transformer and scheduler for HunyuanVideo diffusion.
...
def initialize_pipeline(
    ...
    **kwargs,
    ...
    """Initialize a HunyuanVideoPipeline with optional components.
    ...
    Args:
        ...
        text_encoder: Optional LLaMA encoder override.
        tokenizer_2: Optional CLIP tokenizer override.
        text_encoder_2: Optional CLIP encoder override.
...
def prepare_conditions(
    ...
    **kwargs,
    ...
    """Encode prompts into conditioning embeddings.
    ...
    Returns:
        Dict with prompt embeddings and masks.
    """
    ...
    conditions.update(
        _get_clip_prompt_embeds(tokenizer_2, text_encoder_2, prompt, device, dtype)
    )
...
def prepare_latents(
    ...
    **kwargs,
    ...
    """Encode input frames into VAE latents.
    ...
    image_or_video = image_or_video.permute(
        0, 2, 1, 3, 4
    ).contiguous()  # [B, C, F, H, W] -> [B, F, C, H, W]
    if not precompute:
...
def post_latent_preparation(
    ...
    **kwargs,
    ...
    """Apply VAE scaling factor to precomputed latents.

    Args:
        vae_config: VAE config with scaling_factor.
        latents: Latents tensor.
        **kwargs: Unused additional args.

    Returns:
        Dict with scaled latents.
    """
...
def forward_pass(
    ...
    **kwargs,
    ...
    """Run the HunyuanVideo transformer forward pass.
    ...
    Args:
        ...
        guidance: Guidance conditioning tensor.
        latents: Latents tensor.
...
def validation(
    ...
    **kwargs,
    ...
    Args:
        pipeline: HunyuanVideoPipeline instance.
...
def _get_llama_prompt_embeds(
    ...
    """Tokenize and encode prompts with LLaMA using a template.

    Args:
        tokenizer: LLaMA tokenizer.
        text_encoder: LLaMA text encoder.
        prompt: List of prompts.
        prompt_template: Template dict for formatting prompts.
        device: Target device.
        dtype: Target dtype.
        max_sequence_length: Max token length.
        num_hidden_layers_to_skip: Hidden layers to skip from the top.

    Returns:
        Dict with prompt embeddings and attention masks.
    """
...
def _get_clip_prompt_embeds(
    ...
    """Tokenize and encode prompts with CLIP.
    ...
    Returns:
        Dict with pooled CLIP prompt embeddings.
    """
    ...
    prompt_embeds = text_encoder_2(
        text_inputs.input_ids.to(device), output_hidden_states=False
    ).pooler_output
    prompt_embeds = prompt_embeds.to(dtype=dtype)
