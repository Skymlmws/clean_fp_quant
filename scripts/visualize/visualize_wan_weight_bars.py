...
from scripts.visualize.visualize_wan_activation_surfaces import evenly_spaced_indices, render_bars, selected_sites
...
def selected_linears(spec: str, sites: list[str]) -> list[tuple[str, str]]:
    available = [
        (site, linear)
        for site in sites
        for linear in WAN_LINEAR_TRANSFORM_GROUPS[site]
    ]
